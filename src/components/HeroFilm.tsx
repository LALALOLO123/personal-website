...
/* ---------------------------------------------------------------------------
   ...
   Three assets, per docs/hero-video-plan.md:
     ...
     hold     a short loop that STARTS AND ENDS on the keyframe: the street
              still breathing - distant traffic, shifting light - while the
              camera and the letters do not move at all
     keyframe the same frame all three clips touch, as a still. Poster for
              the loop, and the whole resting state if the loop is missing
     exit     from that frame: the robot comes out from behind the letters,
              walks to the lens, and pulls the view into black

   Every clip is anchored to that one keyframe - the intro ends on it, the
   loop starts and ends on it, the exit starts from it - so continuity is
   structural rather than careful. No join can mismatch, because there is
   only ever one frame at each seam.
...
const HOLD = "/reels/hero-hold.mp4";
...
const HeroFilm = forwardRef<HeroHandle, { active: boolean }>(function HeroFilm({ active }, ref) {
  ...
  return (
    <div className={`film film--${phase} ${hasExit ? "" : "film--fallback"}`} data-active={active}>
      ...
      {hasHold && (
        <video
          className="film__clip film__clip--hold"
          src={HOLD}
          autoPlay
          muted
          loop
